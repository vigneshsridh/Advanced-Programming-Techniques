...
//   The broken (computed) private key for the above is 4105243553
  ...
  //Factorization of n by square root method
  mpz_class sqrt;
  mpz_sqrt(sqrt.get_mpz_t(), rsa.n.get_mpz_t());
  mpz_class factor1 = sqrt;
  if(factor1 % 2 == 0) {
      factor1--;
  }
  while(factor1 > 1) {
  ...
  mpz_class factor2 = rsa.n / factor1;
  mpz_class phi = (factor1 - 1) * (factor2 - 1);
